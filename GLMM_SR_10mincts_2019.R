...
#boxplot for sp detected by each count type in APRIL ONLY
...
# with poisson glm line
spdet_time_ctype <- ggplot(spdet_paired, aes(x=day_of_yr_c, y=sp_detected, 
                                             color=count_type)) +  
  geom_point(shape=1) +
  theme_bw() +
...
spdet_time_ctype_loess <- ggplot(spdet_paired, aes(x=day_of_yr_c, y=sp_detected, 
                                             color=count_type)) +  
  theme_bw() +
  geom_point(shape=1) +
  scale_colour_hue(l=50) + # Use a slightly darker palette than normal
  geom_smooth(method="loess",  # Use loess line instead of glm line
...
## look at species detected per count by count type for each point location
pts_spdet <- ggplot(data = spdet_paired, aes(x = day_of_yr_c, y = sp_detected, 
...
#summary(spdetmod)
#plot(spdetmod)
## this gaussian regression does NOT meet linearity assumption because residuals
## show a distinctive shape. ((WRONG-- this may actually meet assumptions)) 
## so instead we try:  
## 
...
gaus_anova <- anova(null_spdetmod, spdetmod, test = "Chisq")

nullpois_spdetmod <- glm(sp_detected ~ 1, data = spdet_paired, 
                         family = "poisson")
pois_anova <- anova(nullpois_spdetmod, pois_spdetmod, test= "Chisq")
...
gausNoPoly_anova <- anova(spdetmod_noPoly, spdetmod, test = "Chisq")

pois_spdetmod_nopoly <- glm(sp_detected ~ 1 + count_type + wind + rain + noise +
                              day_of_yr_c + rain:count_type + min_past_sun, 
                            data = spdet_paired, family = "poisson")
poisNoPoly_anova <- anova(pois_spdetmod_nopoly, pois_spdetmod, test = "Chisq")
...
### roughly the same as normal poisson model, so we will use normal poisson, 
...
aruid_anova <- anova(aruspdetmod, aruspdetmod2, test = "Chisq")
